...
public class QiscusTextCrawler {
	...
	private final String WWW_PROTOCOL = "www.";

	private QiscusLinkPreviewCallback callback;

	public QiscusTextCrawler() {
	}

	public void makePreview(QiscusLinkPreviewCallback callback, String url) {
	...
	public class GetCode extends AsyncTask<String, Void, Void> {
		...
		private QiscusSourceContent sourceContent = new QiscusSourceContent();
		...
		protected Void doInBackground(String... params) {
			...
			urls = QiscusSearchUrls.matches(params[0]);
				...
				if (isImage(sourceContent.getFinalUrl())
					...
					try {
						...
						if (sourceContent.getTitle().equals("")) {
							String matchTitle = QiscusRegex.pregMatch(
									sourceContent.getHtmlCode(),
									QiscusRegex.TITLE_PATTERN, 2);
	...
	private String getTagContent(String tag, String content) {
		...
		List<String> matches = QiscusRegex.pregMatchAll(content, pattern, 2);
		...
		if (result.equals("")) {
			String matchFinal = QiscusRegex.pregMatch(content, pattern, 2);
	...
	private String cannonicalPage(String url) {
		...
		} else if (url.startsWith(WWW_PROTOCOL)) {
			url = url.substring(WWW_PROTOCOL.length());
		}
	...
	private boolean isImage(String url) {
		return url.matches(QiscusRegex.IMAGE_PATTERN);
	}
	...
	private HashMap<String, String> getMetaTags(String content) {
		...
		List<String> matches = QiscusRegex.pregMatchAll(content,
				QiscusRegex.METATAG_PATTERN, 1);
	...
	private String separeMetaTagsContent(String content) {
		String result = QiscusRegex.pregMatch(content, QiscusRegex.METATAG_CONTENT_PATTERN,
				1);
		return htmlDecode(result);
	}
	...
	private String unshortenUrl(String shortURL) {
		if (!shortURL.startsWith(HTTP_PROTOCOL)
				...
				&& !shortURL.startsWith(WWW_PROTOCOL))
			return "";
